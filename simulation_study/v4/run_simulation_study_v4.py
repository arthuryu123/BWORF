#!/usr/bin/env python3
"""
Simulation Study v4 - Runner Script (Sol/HPC Ready)
...
How to run:
    ...
    #   SLURM_CPUS_ON_NODE / SLURM_CPUS_PER_TASK -> n_cores
...
Outputs
-------
  - v4/outputs_patched/simulation_results_v4[_shardK].csv
  - v4/outputs_patched/run_config_v4[_shardK].json
"""
...
N_ESTIMATORS = 100  # Shared across rf and orf
...
OUTPUTS_DIR = BASE_DIR / "outputs_patched"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Simulation Study v4 Runner")
    parser.add_argument(
        "--n-cores",
        type=int,
        default=None,
        help="Number of parallel cores (default: SLURM_CPUS_ON_NODE or SLURM_CPUS_PER_TASK or 6)",
    )
    parser.add_argument(
        "--shard-id",
        type=int,
...
def get_runtime_config(args):
    ...
    # n_cores: CLI > SLURM_CPUS_ON_NODE > SLURM_CPUS_PER_TASK > 6
    if args.n_cores is not None:
        ...
        n_cores = int(
            os.environ.get(
                "SLURM_CPUS_ON_NODE",
                os.environ.get("SLURM_CPUS_PER_TASK", "6")
            )
        )
...
def evaluate_single(
    ...
    try:
        result["balanced_accuracy"] = float(balanced_accuracy_score(y_test, y_pred))
    except Exception:
...
def main():
    ...
    print("=" * 70)
    print(f"Simulation Study v4 - Runner (rf vs orf vs bworf)")
    print("=" * 70)
    ...
    # Paths
    if cfg["n_shards"] > 1:
        csv_path = OUTPUTS_DIR / f"simulation_results_v4_shard{cfg['shard_id']}.csv"
        config_path = OUTPUTS_DIR / f"run_config_v4_shard{cfg['shard_id']}.json"
    else:
        csv_path = OUTPUTS_DIR / "simulation_results_v4.csv"
        config_path = OUTPUTS_DIR / "run_config_v4.json"
