#!/usr/bin/env python3
"""
Simulation Study v3 — Imbalance Add-On (Sol/HPC Ready)
======================================================

How to run:
    # Local (6 cores default)
    python run_simulation_study_v3_imbalance_addon.py

    # Override cores
    python run_simulation_study_v3_imbalance_addon.py --n-cores 12

    # Slurm array job (sharding)
    python run_simulation_study_v3_imbalance_addon.py --shard-id 0 --n-shards 4
...
Purpose
-------
A focused add-on to the v3 simulation study that examines the effect of
class imbalance (case_control_ratio ∈ {0.2, 0.1}) on four methods:
  - rf:             sklearn RandomForestClassifier
  - orf:            Oblique Random Forest via Treeple
  - bworf_uniform:  BWORF with uniform (standard) bootstrap
  - bworf_weighted: BWORF with class-balanced weighted bootstrap

Grid is deliberately small (fixed p=1000, m=5, signal_ratio=1.0) to keep
runtime manageable while isolating the imbalance effect.

Metrics (expanded vs v3):
  - AUROC (auc)
  - AUPRC / average precision (auprc)
  - F1 score at threshold 0.5 (f1)
  - Balanced accuracy (balanced_acc)
  - Brier score (brier)
  - Causal recovery: mean_rank_causal, median_rank_causal, recall_at_20

Outputs
-------
  - v3/outputs/simulation_results_v3_imbalance_addon.csv
  - v3/outputs/run_config_v3_imbalance_addon.json
"""
...
# ==============================================================================
# IMBALANCE ADD-ON GRID  (Single Source of Truth)
# ==============================================================================
...
M_LIST = [5]                         # fixed
RHO_LIST = [0.2, 0.8]
SIGNAL_RATIO_LIST = [1.0]           # fixed
MAF_TYPE = "fixed"
MAF_FIXED = 0.3
CASE_CONTROL_RATIO_LIST = [0.2, 0.1]  # ← the new factor
...
MASTER_SEED = 12345
VERSION = "v3_imbalance_addon"

# Model hyperparameters — unified across ALL methods
N_ESTIMATORS = 300
...
# Permutation importance settings (same as v3)
PERM_N_REPEATS = 1
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Simulation Study v3 — Imbalance Add-On Runner"
    )
    parser.add_argument(
        "--n-cores",
        type=int,
...
def generate_all_scenarios() -> list:
    """Generate full scenario list from the imbalance add-on grid."""
    scenarios = []
            ...
            for m in sorted(M_LIST):
                for h2 in sorted(H2_LIST):
                    for rho in sorted(RHO_LIST):
                        for sr in sorted(SIGNAL_RATIO_LIST):
                            for ccr in sorted(CASE_CONTROL_RATIO_LIST):
                                scenario = {
                                    "p": p,
                                    "n_samples": n,
                                    "m": m,
...
# ==============================================================================
# Model Creation — FOUR methods
# ==============================================================================

# Methods recognised by this add-on
ALL_METHODS = ["rf", "orf", "bworf_uniform", "bworf_weighted"]
...
def get_proba_class1(model, X: np.ndarray, method: str) -> np.ndarray:
    ...
    if proba.shape[0] != n_samples:
        raise RuntimeError(
            f"{method}: predict_proba has {proba.shape[0]} rows, expected {n_samples}"
        )
    ...
    if not np.allclose(row_sums, 1.0, atol=1e-4):
        print(
            f"  WARNING [{method}]: row sums range "
...
# ==============================================================================
# Metrics — expanded for imbalance study
# ==============================================================================
...
def compute_classification_metrics(
    ...
    """
    Compute all classification metrics relevant to the imbalance study.
    Returns dict with keys: auc, auprc, f1, balanced_acc, brier, status, error_msg.
    """
    ...
    # F1 at threshold=0.5
    try:
        result["f1"] = float(f1_score(y_true, y_pred, zero_division=0.0))
    except Exception:
        result["f1"] = np.nan
...
def evaluate_single(
    ...
    # Classification metrics (expanded)
    m = compute_classification_metrics(y_test, y_proba)
    result["auc"] = m["auc"]
    result["auprc"] = m["auprc"]
    result["f1"] = m["f1"]
    ...
    try:
        importances, _ = permutation_importance_auc(
            model, X_test, y_test, method, perm_seed
        )
...
def write_config(
    config_path: Path, cfg: dict, scenarios: list, shard_scenarios: list
):
    config = {
        "version": VERSION,
        ...
        "grid": {
            ...
            "rho": RHO_LIST,
            "signal_ratio": [
                "inf" if isinstance(x, float) and np.isinf(x) else x
                for x in SIGNAL_RATIO_LIST
            ],
            "maf_type": MAF_TYPE,
            "maf_fixed": MAF_FIXED,
            "case_control_ratio": CASE_CONTROL_RATIO_LIST,
        },
        ...
        "metrics": ["auc", "auprc", "f1", "balanced_acc", "brier",
...
def get_available_methods() -> list:
    ...
    if BWORF_CLASS is not None:
        methods.append("bworf_uniform")
        methods.append("bworf_weighted")
    return methods
...
def main():
    ...
    n_scenarios = (
        len(H2_LIST) * len(P_LIST) * len(N_LIST) * len(M_LIST)
        * len(RHO_LIST) * len(SIGNAL_RATIO_LIST) * len(CASE_CONTROL_RATIO_LIST)
    )
    methods = get_available_methods()
    n_methods = len(methods)
    n_splits = len(HOLDOUT_SEEDS) * N_REPLICATES
    expected_rows = n_scenarios * n_methods * n_splits

    print("=" * 70)
    print(f"Simulation Study {VERSION}")
    print("=" * 70)
    print(f"n_cores: {cfg['n_cores']}")
    print(f"shard: {cfg['shard_id']} / {cfg['n_shards']}")
    print(f"methods: {methods}")
    print(f"case_control_ratios: {CASE_CONTROL_RATIO_LIST}")
    print(f"n_estimators (all methods): {N_ESTIMATORS}")
    print(f"n_replicates: {N_REPLICATES}")
    print(f"expected scenarios: {n_scenarios}")
    print(f"expected total rows: {expected_rows}")
    print()

    # Generate scenarios
    all_scenarios = generate_all_scenarios()
    shard_scenarios = filter_scenarios_by_shard(
        all_scenarios, cfg["shard_id"], cfg["n_shards"]
    )
    ...
    # Paths — VERSION-specific to avoid overwriting v3
    csv_path = OUTPUTS_DIR / f"simulation_results_{VERSION}.csv"
    config_path = OUTPUTS_DIR / f"run_config_{VERSION}.json"
            ...
            for holdout_seed in HOLDOUT_SEEDS:
                for method in methods:
                    ...
                    if key not in completed:
                        tasks.append((
                            scenario, scenario_id, scenario_json,
                            replicate, method, holdout_seed,
                        ))
    ...
    # Smoke test mode
    if cfg["smoke_test"]:
        print("SMOKE TEST: Running first 4 tasks only (one per method)")
        tasks = tasks[:4]
    ...
    for batch_idx in range(n_batches):
        ...
        try:
            ...
            if TerminatedWorkerError is not None and isinstance(
        ...
        for row in results:
            ...
            ccr = row.get("case_control_ratio", "?")
            if status == "ok":
                auc_s = f"{auc:.3f}" if not np.isnan(auc) else "nan"
                auprc_s = f"{auprc:.3f}" if not np.isnan(auprc) else "nan"
                print(
                    f"  {method:15s} ccr={ccr} AUC={auc_s} AUPRC={auprc_s}"
                )
            else:
                print(
                    f"  {method:15s} {status}: {row.get('error_msg', '')[:60]}"
                )
