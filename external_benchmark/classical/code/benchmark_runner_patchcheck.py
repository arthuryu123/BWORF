...
"""
benchmark_runner_patchcheck.py

Fast real-data patch-check runner for BWORF on:
...
# ---------------------------------------------------------------------------
# Import patched BWORF from the same folder as this runner
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Dataset registry: only the two quick-check datasets
# ---------------------------------------------------------------------------
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Quick patch-check runner for BWORF")

    p.add_argument("--data_dir", default="datasets")
    p.add_argument("--out_dir", default="outputs")
    p.add_argument("--datasets", default="heart_failure,thyroid")
    ...
    # MI
    p.add_argument("--bworf_top_k_override", type=int, default=None,
...
def build_preprocessor(spec: dict, X_columns: list[str]) -> Pipeline:
...
def sanitize_proba(P: np.ndarray, n_classes: int, context: str = "") -> np.ndarray:
    ...
    # Normalize all rows, including binary, just to be safe
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray, proba: np.ndarray, task: str, n_classes: int) -> dict[str, float]:
    ...
    if task == "binary":
        ...
        m["auprc_macro_ovr"] = float(
            np.mean([average_precision_score(y_bin[:, k], proba[:, k]) for k in range(n_classes)])
        )
...
def _empty_fold_row(ds_name, seed, fold_id, model_name, task, n_train, n_test, n_classes, metric_names, note, args, top_k_used):
    row: dict = {
...
def run_dataset_model(ds_name: str, spec: dict, model_name: str, seeds: list[int], args: argparse.Namespace, n_jobs: int) -> None:
    ...
    for seed in seeds:
        ...
        for fold_id, (train_idx, test_idx) in enumerate(skf.split(X_df, y)):
            ...
            # Optional MI filtering for the MI arm only.
            if model_name == "bworf_mi":
...
def main() -> None:
    ...
    print("\nPATCH-CHECK BENCHMARK COMPLETE")
