#!/usr/bin/env python3
"""
Multiclass DILI benchmark runner for patched BWORF on Sol.

Locked design:
- Dataset: dili_multiclass.csv
- Features: D001..D777
- Target: y_3class
- Bookkeeping only: USER_ID
- Models:
    rf, orf_style, lr, svm_rbf, xgb, naive_bayes, bworf_no_mi, bworf_mi
- CV: 5 seeds x 10-fold stratified CV
- MI only for bworf_mi, top_k=50, fit on training fold only
- Full split manifest
...
# Import patched BWORF from project code directory at runtime
# The caller should place this script in the same code/ folder.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run patched multiclass DILI benchmark.")
    parser.add_argument("--data_path", required=True, help="Path to dili_multiclass.csv")
    parser.add_argument(
        "--feature_names_path",
        default=None,
        help="Optional path to dili_multiclass_feature_names.csv for reference only",
    )
    parser.add_argument("--output_root", required=True, help="Root directory for run outputs")
    parser.add_argument(
        "--run_name",
        default=None,
        help="Optional run name prefix. Final folder is <run_name>_<timestamp>.",
    )
    ...
    parser.add_argument(
        "--keep_model_stdout",
        action="store_true",
        help="Keep verbose BWORF fit stdout in terminal/log instead of suppressing it.",
    )
    parser.add_argument(
        "--smoke",
        action="store_true",
        help="Smoke test mode: 1 seed, 3 folds, 3 models (rf, naive_bayes, bworf_no_mi).",
    )
...
def select_feature_columns(feature_cols: Sequence[str], df: pd.DataFrame) -> List[str]:
    # locked contract prefers D001..D777 explicitly
...
def fit_imputer(X_train: pd.DataFrame, X_test: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, SimpleImputer]:
...
def preprocess_for_model(
    ...
    X_train_imp, X_test_imp, _ = fit_imputer(X_train_df, X_test_df)
    ...
    # Oblique-family models are also scaled because raw Mold2 values can exceed ~1e30,
    # which makes liblinear split fitting unstable in the patched BWORF code.
    do_scale = model_name in {"lr", "svm_rbf", "orf_style", "bworf_no_mi", "bworf_mi"}
    X_train_proc, X_test_proc = maybe_scale(X_train_imp, X_test_imp, do_scale)
...
def compute_macro_auprc_ovr(y_true: np.ndarray, proba: np.ndarray, class_order: np.ndarray) -> float:
    ...
    for i, cls in enumerate(class_order):
        ...
        values.append(average_precision_score(y_bin, proba[:, i]))
...
def compute_fold_metrics(y_true: np.ndarray, y_pred: np.ndarray, proba: np.ndarray) -> Dict[str, float]:
    metrics = {
        ...
        "macro_auroc_ovr": float(
            roc_auc_score(y_true, proba, multi_class="ovr", average="macro", labels=CLASS_ORDER)
        ),
...
def build_confusion_payload(oof_df: pd.DataFrame) -> Dict[str, object]:
    payload: Dict[str, object] = {"per_seed": {}, "pooled": {}}
...
def build_sanity_payload(
    ...
    duplicate_oof_key_count = int(
        oof_df.duplicated(subset=["sample_index", "seed", "model_name"]).sum()
    )
    proba_sums = (
        oof_df[["proba_0", "proba_1", "proba_2"]].sum(axis=1).to_numpy(dtype=float)
        if len(oof_df) > 0
        else np.array([1.0])
    )
...
def run_benchmark(args: argparse.Namespace) -> Path:
    ...
    feature_cols = select_feature_columns(feature_cols, df)
    _feature_name_ref = maybe_read_feature_name_map(args.feature_names_path)
    ...
    for seed in seeds:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(cv.split(X_df, y), start=1):
            ...
            for model_name in models:
                ...
                model = build_model(model_name, seed=seed, n_jobs=args.n_jobs)
                y_pred, proba_raw, fit_time, pred_time = fit_and_predict(
                    model,
                    processed.X_train,
                    y_train,
                    processed.X_test,
                    keep_stdout=args.keep_model_stdout,
                )
                proba = align_proba_columns(
                    proba_raw,
                    model=model,
                    class_order=CLASS_ORDER,
                    fallback_classes=np.unique(y_train),
                )
    ...
    mi_df = pd.DataFrame(mi_rows_all)
        ...
        json.dump(
            {
                ...
                "notes": {
                    ...
                    "oblique_models_scaled": True,
                    "oblique_scaling_rationale": "Raw Mold2 values can exceed ~1e30; fold-local scaling stabilizes liblinear-based oblique split fitting.",
                },
                "hyperparameters": {
                    "rf": {
                        "n_estimators": 100,
